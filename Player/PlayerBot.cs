...
namespace MCForge
{
    public class PlayerBot
    {
        ...
        void UpdatePosition()   //Im going to avoid touching this unless nessesary
        {
            ...
            if ((changed & 4) != 0)
            {
                msg = OpCode.Teleport; buffer = new byte[9]; buffer[0] = id;
            ...
            {
                msg = OpCode.Move; buffer = new byte[4]; buffer[0] = id;
            ...
            {
                msg = OpCode.Rotate; buffer = new byte[3]; buffer[0] = id;
                buffer[1] = rot[0]; buffer[2] = rot[1];
            }
            else if (changed == 3)
            {
                msg = OpCode.MoveRotate; buffer = new byte[6]; buffer[0] = id;
                buffer[1] = (byte)(pos[0] - oldpos[0]);
                buffer[2] = (byte)(pos[1] - oldpos[1]);
                buffer[3] = (byte)(pos[2] - oldpos[2]);
