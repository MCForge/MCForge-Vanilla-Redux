...
namespace MCForge
{
    ...
    /// <summary> Drawing/clipping mode of IsoCat level renderer. </summary>
    public enum IsoCatMode
    {
        /// <summary> Normal isometric view. </summary>
        Normal,

        /// <summary> Isometric view with the outermost layer of blocks stripped (useful for boxed levels). </summary>
        Peeled,

        /// <summary> Isometric view with a front-facing quarter of the level cut out (to show level cross-section). </summary>
        Cut,

        /// <summary> Only a specified chunk of the level is drawn. </summary>
        Chunk
    }


    /// <summary> Isometric level renderer, tightly integrated with BackgroundWorker.
    /// Creates a bitlevel of the level. Every IsoCat instance is single-use. </summary>
    unsafe public sealed class IsoCat
    {
        static readonly ushort[] Tiles, ShadowTiles;
        static readonly int TileX, TileY;
        static readonly int MaxTileDim, TileStride;

        static readonly Bitmap Tileset, TilesetShadowed;

        static IsoCat()
        {
            if (!Directory.Exists("extra")) Directory.CreateDirectory("extra");
            if (!File.Exists("extra/Tileset.png"))
                using (WebClient w = new WebClient())
                    w.DownloadFile("http://updates.mcdawn.com/Tileset.png", "extra/Tileset.png");
            if (!File.Exists("extra/TilesetShadowed.png"))
                using (WebClient w = new WebClient())
                    w.DownloadFile("http://updates.mcdawn.com/TilesetShadowed.png", "extra/TilesetShadowed.png");
            Tileset = new Bitmap(Bitmap.FromFile("extra/Tileset.png"));
            TilesetShadowed = new Bitmap(Bitmap.FromFile("extra/TilesetShadowed.png"));
            using (Bitmap tilesBmp = Tileset)
            {
                TileX = tilesBmp.Width / 50;
                TileY = tilesBmp.Height;
                TileStride = TileX * TileY * 4;
                Tiles = new ushort[50 * TileStride];
            ...
            using (Bitmap stilesBmp = TilesetShadowed)
            {

                ShadowTiles = new ushort[50 * TileStride];
