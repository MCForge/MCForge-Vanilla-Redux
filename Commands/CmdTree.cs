...
namespace MCForge.Commands
{
    public class CmdTree : Command
    {
        ...
        void AddTree(Player p, ushort x, ushort y, ushort z, ushort type)
        {
            Server.MapGen.AddTree(p.level, x, y, z, p.random, true, true, p);
            if (!p.staticCommands) p.ClearBlockchange();
        }
        void AddNotchTree(Player p, ushort x, ushort y, ushort z, ushort type)
        {
            Server.MapGen.AddNotchTree(p.level, x, y, z, p.random, true, true, p);
            if (!p.staticCommands) p.ClearBlockchange();
        }
        void AddNotchBigTree(Player p, ushort x, ushort y, ushort z, ushort type)
        {
            Server.MapGen.AddNotchBigTree(p.level, x, y, z, p.random, true, true, p);
            if (!p.staticCommands) p.ClearBlockchange();
        }
        void AddNotchPineTree(Player p, ushort x, ushort y, ushort z, ushort type)
        {
            Server.MapGen.AddNotchPineTree(p.level, x, y, z, p.random, true, true, p);
            if (!p.staticCommands) p.ClearBlockchange();
        }
        void AddNotchSwampTree(Player p, ushort x, ushort y, ushort z, ushort type)
        {
            Server.MapGen.AddNotchSwampTree(p.level, x, y, z, p.random, true, true, p);
            if (!p.staticCommands) p.ClearBlockchange();
        }
        void AddCactus(Player p, ushort x, ushort y, ushort z, ushort type)
        {
